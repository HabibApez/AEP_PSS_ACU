...
E_WindowStateMachine re_State = WINDOW_IDLE;
E_ButtonState re_ButtonStatus = NONE_BUTTON_PRESS;
...
void windowlifter_IdleState(void){
  if(re_ButtonStatus == DOWN_BUTTON_PRESS){
    re_State = ONE_TOUCH_DOWN;
  }			
  else if(re_ButtonStatus == UP_BUTTON_PRESS){
    re_State = ONE_TOUCH_UP;
  }	
  else if((re_ButtonStatus == ANTIPINCH_BUTTON_PRESS) && ((re_State == ONE_TOUCH_UP) || (re_State == UP_NORMAL))){
    re_State = ANTIPINCH;
  }
  else{

    re_State = WINDOW_IDLE;
  }
}
...
void windowlifter_OneTouchUpState(void){                        //ONE_TOUCH_UP
  ...
  if(re_ButtonStatus == UP_BUTTON_PRESS){
    rub_UpButtonCounter++;
  ...
  if(re_ButtonStatus == DOWN_BUTTON_PRESS){                    // Change window moving direction
    rub_UpButtonCounter = ZERO_MS;
    leds_TurnOffUpLED();
    re_State = DOWN_NORMAL;
  }
}
...
void windowlifter_UpNormalState(void){
  if(re_ButtonStatus == ANTIPINCH_BUTTON_PRESS){
    leds_TurnOffUpLED();
    re_State = ANTIPINCH;
  }
  else{
...
void windowlifter_OneTouchDownState(void){                        //ONE_TOUCH_DOWN
  ...
  if(re_ButtonStatus == UP_BUTTON_PRESS){                    // Change window moving direction
    rub_DownButtonCounter = ZERO_MS;
    leds_TurnOffDownLED();
    re_State = UP_NORMAL;
  }
